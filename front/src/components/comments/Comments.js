      ...
      <div className="flex flex-col justify-between p-2 bg-sky-100 shadow-lg rounded-lg m-2 w-72 h-80 mb-40">
        <div className="flex justify-between w-full">
          <div className="flex items-center">
            <img className="w-6 h-6 rounded-full mr-2 ml-2" src={img} alt="프로필" />
          ...
          <div className="flex items-center justify-between w-9 h-6 rounded-lg p-1 bg-white mr-2 ">
            ...
            <span className="text-xs font-bold mr-1"> {comment.rating * 0.5}</span>
          </div>
        </div>
        ...
        <div className="flex justify-end w-full">
          <button className="text-xs font-bold bg-white hover:bg-sky-200 p-1 mr-2">
            수정
          </button>
          <button onSubmit={handleDeleteSubmit} className="text-xs font-bold bg-white hover:bg-sky-200 p-1">
            삭제
          </button>
        </div>
      </div>
...
const Comments = ({ commentListResponseDtos }) => {
  return (
    <div className="px-44">
      ...
      <div className="flex flex-wrap m-2 mb-10">
        {commentListResponseDtos.map((comment) => (
          <div className="p-2 w-1/4 min-w-[w-72]">
