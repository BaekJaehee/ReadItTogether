      ...
      {books && books.map((book) => (
        <div key={book.bookId} className="relative group text-center">
          ...
          <div className="m-3 cursor-pointer" onClick={() => handleClickBook(book.bookId)}>
            <img src={book.cover} alt={book.title} className="w-[310px] h-[440px]" />
            <p className="m-1 text-sm font-bold">{book.title}</p>
          </div>
          <button 
            className="bg-white border border-green-300 hover:bg-green-300 hover:text-white rounded-lg text-green-500 text-xs py-2 px-4 mt-2 transition-colors duration-300"
