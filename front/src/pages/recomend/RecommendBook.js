...
const RecommendBook = () => {
  ...
  <div className="flex items-center space-x-2 relative"> {/* 여기에 relative 추가 */}
    {index > 0 && (
      <button
        onClick={moveLeft}
        className="border-2 rounded-full absolute left-6 transform -translate-x-1/2" // 수정된 부분
    ...
    {index < images.length - 5 && (
      <button
        onClick={moveRight}
        className="border-2 rounded-full absolute right-6 transform translate-x-1/2" // 수정된 부분
        style={{ top: '50%', transform: 'translate(50%, -50%)' }} // 버튼을 세로 중앙에 위치시킵니다.
      >
