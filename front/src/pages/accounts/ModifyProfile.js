...
const ModifyProfile = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center pt-32">
      <form onSubmit={handlePasswordSubmit} className="flex flex-col gap-4 mb-8">
        <label htmlFor="oldPassword" className="text-gray-700">현재 비밀번호</label>
        <input
          type="password"
          value={oldPassword}
        ...
        />
        <label htmlFor="newPassword" className="text-gray-700">새로운 비밀번호</label>
        <input
          type="password"
          value={newPassword}
        ...
        <label htmlFor="newPasswordConfirm" className="text-gray-700">새로운 비밀번호 확인</label>
        <input
          type="password"
        ...
        {/* 비밀번호 변경 버튼 */}
        <button type="submit" disabled={!isPasswordOK || !isPasswordMatch} className={`bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 ${(!isPasswordOK || !isPasswordMatch) && 'opacity-50 cursor-not-allowed'}`}>
          비밀번호 변경
        </button>
      </form>
      <br/>
      <button className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600" onClick={() => setIsOpenModal(true)}>
        탈퇴하기
      </button>
      {isOpenModal && (
        <DisableModal onClose={() => setIsOpenModal(false)}/>
      )}
    </div>
  );
}
