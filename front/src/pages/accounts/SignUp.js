...
const SignUp = () => {
  ...
  const handleCheckEmailCode = async () => {
    try {
      await CheckCode(email, emailCode);
      alert('인증되었습니다');
    } catch (error) {
      alert('인증에 실패했습니다');
      console.error(error);
    ...
    <div className="max-w-md mx-auto my-8 p-4 border border-gray-300 rounded shadow">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center">
            <input type="text" id="emailCertificate" name="emailCode" value={emailCode} className="border border-gray-300 px-2 py-1 flex-grow mr-3" />
